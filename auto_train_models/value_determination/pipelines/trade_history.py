...
def _run_surplus(start: int, end: int, force: bool) -> None:
    ...
    from value_determination.pipelines.history_surplus import compute_all_surpluses

    logger.info("╔═══════════════════════════════════════════════════╗")
    logger.info("║  Phase 2 — Surplus Values                        ║")
    logger.info("╚═══════════════════════════════════════════════════╝")

    snap_start = start + _H.SNAPSHOT_LAG
    snap_end   = end   + _H.SNAPSHOT_LAG
...
def _run_timeline() -> None:
    ...
    from value_determination.pipelines.history_timeline import generate_timeline
...
def main(argv: list[str] | None = None) -> None:
    ...
    # --- Year range ---
    parser.add_argument(
        "--start", type=int, default=_H.CUTOFF_START,
        help=f"First cutoff year (default: {_H.CUTOFF_START})",
    )
    parser.add_argument(
        "--end", type=int, default=_H.CUTOFF_END,
        help=f"Last cutoff year (default: {_H.CUTOFF_END})",
    )

...
class PitcherSPConfig:
    """Configuration for starting pitcher model"""
    ...
    def get_data_config():
        return DataConfig(
            ...
            start_season=1950,
            min_pa=30,  # MIN_IP threshold for pitchers
            train_ratio=0.8,
            valid_ratio=0.19,
            random_seed=42
        )
    ...
    def get_model_config():
        return {
            'input_size': len(PitcherSPConfig.INPUT_FEATURES),
            'output_size': len(PitcherSPConfig.INPUT_FEATURES),
            'hidden_size': 512,
            'num_layers': 6,
            'num_heads': 4,
            'bidirectional': True,
